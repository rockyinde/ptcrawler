...
def process(client, line):
  ...
  body = video  # v2
  udate = int(round(time.time() * 1000))

  # upload the video
  print 'uploading %s to DDB\n' % (vid_id)
  saveVideoV2 (vid_id,udate,title,body['likeCount'],body['viewsCountInt'],body['thumbsUpPercentage'],body['publishDate']['value'],"{}",body)

  print 'uploading %s to ESS\n' % (vid_id)
  saveVideoToESS(udate,vid_id,title,body_v1)

def run():

  client = get_service()

  vid_file = open('/home/akshar/Documents/dubwood/videos','r')
